...
const PortalTarget = {
  name: 'vue-customtreeselect--portal-target',
  inject: ['instance'],
  ...
  render() {
    const { instance } = this;
    const portalTargetClass = ['vue-customtreeselect__portal-target', instance.wrapperClass];
...
export default {
  ...
  render() {
    if (!placeholder) {
      placeholder = (
        h('div', { class: 'vue-customtreeselect__menu-placeholder' })
      );
    }
